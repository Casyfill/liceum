...
# Philipp: все что ниже довольно сложно читать :-(

def check_letter(let, w, dict, z, x, array):  # Philipp: аргрументы названы абстрактно, докстринга нет
    let.upper()  # Philipp: никакого эффекта на let не делает
    array.append(let)
    if let in w:
        dict[let] = 1  # Philipp: лучше dict не использовать - это же название встроенной функции
        z += 1
        return 1
    elif x == 6:
        print(pics[x])
...
def main():  # changed by Philipp Kats for consistency
    ...
    while remaining_attempts != 7:
        ...
        if check_guessed_letters == len(word):
            ...
            # Philipp: так (выше) конечно можно делать, но не нужно - используйте f-strings@
            exit()
        print()
